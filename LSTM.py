...
# Create a helper function for the reshaping of the x and y variables. We want to predict the next day's electricity price based on 1 day of data. So we have to create sequences of the desired length.
def create_sequences(data, target, seq_length, step = 24):
    X_sequences, y_sequences = [], []
    for i in range(0, len(data), step):
        X_seq = data[i:i+seq_length]
        y_seq = target[i:i + seq_length]
        X_sequences.append(X_seq)
        y_sequences.append(y_seq)
    return np.array(X_sequences), np.array(y_sequences)

# Reshape the data to be 3-dimensional in the form [samples, timesteps, features].
X_train_reshaped, y_train_reshaped = create_sequences(X_train_scaled, y_train_scaled, 24)
...
def objective(params):
    ...
    checkpoint = ModelCheckpoint('best_model_weights1.h5', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
    ...
    model.load_weights('best_model_weights1.h5')
...
final_model.load_weights('final_model_weights1.h5')
